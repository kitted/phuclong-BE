...
import {
  ...
  LeadQueryDto,
  UpdateLeadDto,
} from '../../../collection/leads/dtos/leads.dto';
import { AuthRequest } from '../../../collection/auth/interfaces/authRequest.interface';
@WarehouseController(['leads'])
export class LeadsController {
  constructor(private readonly service: LeadsService) {}
  private actor(req: AuthRequest): any {
    const value: any = (req.user as any)?._doc || req.user || {};
    return {
  ...
  @Get() list(@Query() q: LeadQueryDto) {
    return this.service.list(q);
  }
  @Get(':id') detail(@Param('id', ParseIdPipe) id: ID) {
    return this.service.findOne(String(id));
  }
  @Post() create(@Body() dto: CreateLeadDto, @Req() req: AuthRequest) {
  ...
  @Delete(':id') @AdminOnly() remove(@Param('id', ParseIdPipe) id: ID) {
    ...
    FileInterceptor('file', {
      ...
      fileFilter: (_req, file, callback) => {
        if (!['image/jpeg', 'image/png', 'image/webp'].includes(file.mimetype))
          return callback(
            new BadRequestException('Chỉ hỗ trợ ảnh JPEG, PNG hoặc WebP'),
            false,
          );
        callback(null, true);
      },
  ...
  uploadImage(@Param('id', ParseIdPipe) id: ID, @UploadedFile() file: any) {
    return this.service.uploadImage(String(id), file);
  }
  @Post(':id/interactions') interact(
    @Param('id', ParseIdPipe) id: ID,
    @Body() dto: CreateLeadInteractionDto,
    @Req() req: AuthRequest,
  ) {
    return this.service.interact(String(id), dto, this.actor(req));
  }
}
