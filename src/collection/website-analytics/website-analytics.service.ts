...
const trim = (value: unknown, max = 140) =>
...
export class WebsiteAnalyticsService {
  ...
  private safeMetadata(
    ...
    for (const [key, value] of Object.entries(input || {}).slice(0, 20)) {
      ...
      if (!safeKey || !['string', 'number', 'boolean'].includes(typeof value))
        continue;
  ...
  async track(
    ...
    if (duplicate)
      return {
        data: { id: duplicate._id, accepted: true, deduplicated: true },
      };
  ...
  async createLead(dto: CreateWebsiteLandingLeadDto) {
    ...
    if (
      ...
      phone.length < 9 ||
      phone.length > 11 ||
      !trim(dto.sessionId, 100) ||
      !trim(dto.landingKey, 80)
    )
    ...
    const data = await this.leads.create({
      name: trim(dto.name, 120),
      phone,
      province: trim(dto.province, 100) || undefined,
      role: trim(dto.role, 60) || undefined,
      landingKey: trim(dto.landingKey, 80),
      sessionId: trim(dto.sessionId, 100),
      source: trim(dto.source, 100) || 'direct',
      medium: trim(dto.medium, 100) || undefined,
    ...
    return {
      data: { id: data.id, status: data.status, createdAt: data.createdAt },
    };
  }
  ...
  private period(query: WebsiteAnalyticsQueryDto) {
    ...
    const from = query.from
      ? new Date(`${query.from}T00:00:00.000+07:00`)
  ...
  async report(query: WebsiteAnalyticsQueryDto): Promise<any> {
    ...
    const [
      ...
      this.events.aggregate([
        ...
        { $sort: { eventType: 1, visits: -1, pagePath: 1 } },
        { $limit: 200 },
      ]),
      this.events.aggregate([
        { $match: { ...filter, eventType: WebsiteEventType.PAGE_VIEW } },
        {
          $group: {
            _id: {
              source: { $ifNull: ['$source', 'direct'] },
              medium: { $ifNull: ['$medium', ''] },
            },
            visits: { $sum: 1 },
            sessions: { $addToSet: '$sessionId' },
          },
      ...
      this.events.distinct('visitorId', {
        ...filter,
        visitorId: { $nin: [null, ''] },
      }),
    ...
    const counts = Object.fromEntries(
      eventCounts.map((row) => [row._id, row.value]),
    );
    const visits = Number(counts[WebsiteEventType.PAGE_VIEW] || 0);
    const forms = Number(
      counts[WebsiteEventType.FORM_SUBMIT] || leadCount || 0,
    );
    const value = (eventType: WebsiteEventType) =>
      Number(counts[eventType] || 0);
    return {
      data: {
        period: { from, to },
        summary: {
          ...
          qrVisits: value(WebsiteEventType.QR_VISIT),
          leads: leadCount,
          conversionRate: visits
            ? Math.round((forms / visits) * 10000) / 100
            : 0,
        },
        funnel: [
          {
            event: WebsiteEventType.PAGE_VIEW,
            label: 'Lượt vào trang',
            value: visits,
          },
          {
            event: WebsiteEventType.PRODUCT_DETAIL_VIEW,
            label: 'Xem sâu sản phẩm',
            value: value(WebsiteEventType.PRODUCT_DETAIL_VIEW),
          },
          {
            event: WebsiteEventType.VIDEO_PLAY,
            label: 'Phát video',
            value: value(WebsiteEventType.VIDEO_PLAY),
          },
          {
            event: WebsiteEventType.OFFER_VIEW,
            label: 'Xem gói test',
            value: value(WebsiteEventType.OFFER_VIEW),
          },
          {
            event: WebsiteEventType.ZALO_CLICK,
            label: 'Bấm Zalo',
            value: value(WebsiteEventType.ZALO_CLICK),
          },
          {
            event: WebsiteEventType.FORM_SUBMIT,
            label: 'Gửi form',
            value: forms,
          },
        ],
