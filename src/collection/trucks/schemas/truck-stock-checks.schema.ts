...
export enum TruckStockCheckStatus {
  MATCHED = 'MATCHED',
  SHORTAGE = 'SHORTAGE',
  SURPLUS = 'SURPLUS',
...
export class TruckStockCheckItem {
  ...
  @prop() unit?: string;
  @prop() systemQuantity?: number;
  @prop({ min: 0 }) actualQuantity?: number;
  @prop() differenceQuantity?: number;
  @prop({ enum: TruckStockCheckStatus, required: true })
  status: TruckStockCheckStatus;
  @prop() note?: string;
  @prop() rowNumber?: number;
}
@index({ truckId: 1, comparedAt: -1 })
@index({ syncIdempotencyKey: 1 }, { unique: true, sparse: true })
export class TruckStockChecks extends BaseModel {
  ...
  @prop({ type: () => [TruckStockCheckItem], default: [] })
  items: TruckStockCheckItem[];
