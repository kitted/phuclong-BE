...
export enum TruckInventoryBackupSource {
  STOCK_CHECK_SYNC = 'STOCK_CHECK_SYNC',
  BEFORE_RESTORE = 'BEFORE_RESTORE',
}
export class TruckInventoryBackupItem {
...
@index({ truckId: 1, createdAt: -1 })
@index({ code: 1 }, { unique: true })
@index({ restoreIdempotencyKey: 1 }, { unique: true, sparse: true })
export class TruckInventoryBackups extends BaseModel {
  ...
  @prop({ enum: TruckInventoryBackupSource, required: true })
  sourceType: TruckInventoryBackupSource;
  @prop() stockCheckId?: string;
  @prop({ type: () => [TruckInventoryBackupItem], default: [] })
  items: TruckInventoryBackupItem[];
  @prop({ required: true, min: 0 }) totalProducts: number;
  @prop({ required: true }) totalQuantity: number;
  @prop({ required: true }) totalCostValue: number;
  @prop({ required: true }) totalSellValue: number;
...
export class TruckInventoryBackupCounters {
