...
export class EmployeeKpisService {
  ...
  async findAll(query: EmployeeKpiQueryDto): Promise<any> { const filter: any = { isDeleted: false }; if (query.employeeId) filter.employeeId = query.employeeId; if (query.status) filter.status = query.status; if (query.search) filter.$or = [{ code: { $regex: query.search, $options: 'i' } }, { name: { $regex: query.search, $options: 'i' } }, { employeeName: { $regex: query.search, $options: 'i' } }]; if (query.from) filter.to = { $gte: new Date(query.from) }; if (query.to) filter.from = { $lte: new Date(query.to) }; const page = +query.page || 1, limit = +query.limit || 20; const [data, total] = await Promise.all([this.model.find(filter).sort({ from: -1 }).skip((page - 1) * limit).limit(limit).lean(), this.model.countDocuments(filter)]); return { data, meta: { page, limit, total, totalPages: Math.ceil(total / limit) } }; }
  async findOne(id: string): Promise<any> { const doc: any = await this.model.findOne({ _id: id, isDeleted: false }).lean(); if (!doc) throw new NotFoundException('Không tìm thấy KPI'); return { data: doc }; }
  ...
  async evidence(id: string, query: EmployeeKpiEvidenceQueryDto): Promise<any> {
    const { data: kpi }: any = await this.findOne(id); const targetIndex = Number(query.targetIndex);
    ...
    const invoices: any[] = await this.invoices.find(base).sort({ date: -1 }).populate('customerId', 'code name phone').lean();
