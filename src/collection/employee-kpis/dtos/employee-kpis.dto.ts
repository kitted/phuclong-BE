...
export class EmployeeKpiTargetDto {
  ...
  @IsOptional() @IsNumber() @Min(0) referenceUnitPrice?: number;
  @IsOptional() @IsNumber() @Min(0) revenueTarget?: number;
...
export class CreateEmployeeKpiDto {
  ...
  @Type(() => EmployeeKpiTargetDto)
  targets: EmployeeKpiTargetDto[];
  @IsOptional() @IsEnum(EmployeeKpiStatus) status?: EmployeeKpiStatus;
  @IsOptional() @IsString() note?: string;
  @IsOptional() @IsEnum(['FIXED_RANGE', 'CUSTOM_MONTHLY']) cycleType?:
    | 'FIXED_RANGE'
    | 'CUSTOM_MONTHLY';
  @IsOptional() @Min(1) @Max(31) startDay?: number;
  @IsOptional() @Min(1) @Max(31) endDay?: number;
}
...
export class ChangeEmployeeKpiStatusDto {
  @IsEnum(EmployeeKpiStatus) status: EmployeeKpiStatus;
  @IsOptional() @IsString() note?: string;
}
