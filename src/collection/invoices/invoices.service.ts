  ...
  if (actor.role === RoleEnum.STAFF) {
    if (!actor.id || !Types.ObjectId.isValid(actor.id))
      throw new ForbiddenException(
        'Không xác định được tài khoản nhân viên hiện tại',
      );
    if (requestedId && String(requestedId) !== String(actor.id))
      throw new ForbiddenException(
        'Nhân viên chỉ được tạo hóa đơn dưới chính tài khoản của mình',
      );
    return String(actor.id);
  }
  if (!requestedId)
    throw new BadRequestException('Vui lòng chọn nhân viên bán hàng');
  if (!Types.ObjectId.isValid(requestedId))
    throw new BadRequestException('salespersonId không hợp lệ');
  return String(requestedId);
}

export function calculateInvoiceDebtAllocation(
  receivedAmount: number,
  grandTotal: number,
  customerDebtBefore: number,
) {
  const paidAmount = Math.min(receivedAmount, grandTotal);
  const existingDebtPaidAmount = Math.min(
    customerDebtBefore,
    Math.max(0, receivedAmount - grandTotal),
  );
  ...
  return {
    ...
    debtAmount,
    customerDebtAfter: customerDebtBefore + debtAmount - existingDebtPaidAmount,
  };
}
...
export class InvoicesService {
  constructor(
    ...
    private readonly promotionModel: ReturnModelType<typeof Promotions>,
    @InjectModel(Vouchers)
    private readonly voucherModel: ReturnModelType<typeof Vouchers>,
    @InjectModel(InvoiceCounters)
    private readonly counterModel: ReturnModelType<typeof InvoiceCounters>,
    @InjectModel(Categories)
    private readonly categoryModel: ReturnModelType<typeof Categories>,
    @InjectModel(PromotionActivations)
    private readonly activationModel: ReturnModelType<
      typeof PromotionActivations
    >,
    ...
    private readonly debtPaymentCounterModel: ReturnModelType<
      typeof DebtPaymentCounters
    >,
  ...
  private async calculate(
    ...
    const requested = dto.items.map((item) => {
      ...
      if (
        ...
        (!Number.isFinite(Number(item.unitPriceOverride)) ||
          Number(item.unitPriceOverride) <= 0)
      )
        throw new BadRequestException('Giá bán tùy chỉnh phải lớn hơn 0');
      return {
    ...
    const categoryIds = [
      ...new Set(
        products
          .map((product) => product.categoryId && String(product.categoryId))
          .filter(Boolean),
      ),
    ];
    ...
    const items = requested.map((item) => {
      ...
      const catalogPrice = Number(product.sellPrice) || 0;
      const unitPriceOverride = item.unitPriceOverride;
      const price = unitPriceOverride ?? catalogPrice;
      return {
        productId: item.productId,
        ...
        catalogPrice,
        price,
        priceOverridden: unitPriceOverride !== undefined,
        unitPriceOverride,
        priceOverriddenBy:
          unitPriceOverride !== undefined ? actorId : undefined,
  ...
  private async directGiftLines(
    gifts: Array<{ productId: string; qty: number }> | undefined,
    session: ClientSession,
  ) {
    ...
    if (products.length !== requested.length)
      throw new BadRequestException(
        'Một hoặc nhiều sản phẩm quà tặng không tồn tại',
      );
    const productMap = new Map(
      products.map((product) => [String(product._id), product]),
    );
    return requested.map((item) => {
      ...
      return {
        ...
        unit: product.unit || '',
        qty: item.qty,
        catalogPrice: Number(product.sellPrice) || 0,
        price: 0,
        priceOverridden: false,
        originalPrice: Number(product.sellPrice) || 0,
        costPrice: Number(product.costPrice) || 0,
        lineTotal: 0,
    ...
    return {
      data: {
        ...
        promotion: calculated.promotion
          ? {
              id: String(calculated.promotion._id),
              code: calculated.promotion.code,
              name: calculated.promotion.name,
  ...
  async applyGiftPromotion(dto: ApplyGiftPromotionDto) {
    const calculated = await this.calculate({ ...dto });
    const applied = await this.ruleEngine.apply(
      dto.promotionId,
      calculated.items,
      dto.giftSelections,
    );
    return {
      data: {
        ...
        promotionApplication: {
          promotionId: String(applied.promotion._id),
          promotionCode: applied.promotion.code,
          promotionName: applied.promotion.name,
          applicationCount: applied.applicationCount,
          matchedConditions: applied.matchedConditions,
          gifts: applied.gifts,
        },
        items: [
          ...calculated.items,
          ...applied.gifts.map((gift) => ({
            ...gift,
            price: 0,
            originalPrice: gift.sellPrice,
            lineTotal: 0,
            lineType: InvoiceLineType.GIFT,
          })),
        ],
      ...
      map.set(
        ...
        existing
          ? {
              ...
              referenceCode: payment.referenceCode || existing.referenceCode,
            }
  ...
  private splitPayments(payments: any[], invoiceAmount: number) {
    ...
    for (const payment of payments) {
      ...
      if (invoicePart > 0)
        invoicePayments.push({ ...payment, amount: invoicePart });
  ...
  async create(dto: CreateInvoiceDto, actor: Actor = {}): Promise<any> {
    ...
    if (dto.customerId && dto.newCustomer)
      throw new BadRequestException(
        'Không được gửi đồng thời customerId và newCustomer',
      );
    if (dto.newCustomer && !dto.newCustomer.name?.trim())
    ...
    if ((dto.promotionApplications || []).length > 1)
      throw new BadRequestException(
        'Mỗi hóa đơn chỉ được áp dụng một chương trình tặng quà',
      );
    if (dto.gifts?.length && dto.promotionApplications?.length)
      throw new BadRequestException(
        'Không thể dùng đồng thời quà tặng trực tiếp và chương trình tặng quà',
      );
    const session = await this.connection.startSession();
    let response: any;
    try {
      await session.withTransaction(async () => {
        const salesperson: any = await this.userModel
          .findOne({
            _id: salespersonId,
            role: RoleEnum.STAFF,
        ...
        if (!salesperson)
          throw new BadRequestException(
            'Nhân viên bán hàng không hoạt động hoặc không tồn tại',
          );
        let customer: any = dto.customerId
        ...
        if (dto.newCustomer) {
          const phones = String(dto.newCustomer.phone || '')
            .split(/[,;|/]+/)
        ...
        const giftApplication = giftRequest
          ? await this.ruleEngine.apply(
              giftRequest.promotionId,
              calculated.items,
              giftRequest.giftSelections,
              session,
              false,
            )
          : null;
        const giftLines = giftApplication
          ? giftApplication.gifts.map((gift) => ({
              ...gift,
              catalogPrice: gift.sellPrice,
              price: 0,
              priceOverridden: false,
              originalPrice: gift.sellPrice,
              lineTotal: 0,
        ...
        if (dto.applyExcessToDebt && !customer)
          throw new BadRequestException(
            'Chỉ khách hàng CRM mới được thanh toán kèm trừ nợ cũ',
          );
        if (!dto.applyExcessToDebt && receivedAmount > calculated.grandTotal)
          throw new BadRequestException(
            'Số tiền thanh toán không được vượt tổng hóa đơn',
          );
        if (
          dto.applyExcessToDebt &&
          receivedAmount > calculated.grandTotal + Number(customer?.debt || 0)
        )
          throw new BadRequestException(
            'Số tiền nhận vượt tổng hóa đơn và công nợ cũ',
          );
        const allocation = calculateInvoiceDebtAllocation(
          receivedAmount,
          calculated.grandTotal,
          Number(customer?.debt || 0),
        );
        const paidAmount = allocation.paidAmount;
        const existingDebtPaidAmount = dto.applyExcessToDebt
          ? allocation.existingDebtPaidAmount
          : 0;
        const debtAmount = allocation.debtAmount;
        const customerDebtAfter = customer
          ? Number(customer.debt || 0) + debtAmount - existingDebtPaidAmount
          : 0;
        ...
        const code =
          ...
          `HD-${day.slice(2)}-${String(counter.sequence).padStart(6, '0')}`;
        if (await this.model.exists({ code }).session(session))
          throw new ConflictException('Mã hóa đơn đã tồn tại');
        let giftCode: string | undefined;
        if (directGiftLines.length) {
          const giftCounter: any = await this.counterModel.findOneAndUpdate(
            { key: `INVOICE_GIFT_${day}` },
            { $inc: { sequence: 1 } },
            { upsert: true, new: true, session },
          );
        ...
        if (dto.sourceType === 'warehouse') {
          for (const [productId, lines] of inventoryGroups) {
            ...
            const before: any = await this.productModel.findOneAndUpdate(
              { _id: productId, isDeleted: false, stock: { $gte: totalQty } },
              { $inc: { stock: -totalQty } },
              { new: false, session },
            );
            if (!before)
              throw new ConflictException({
                code: lines.some(
                  ...
                  : 'INSUFFICIENT_STOCK',
                message: 'Tổng số lượng bán và quà vượt tồn kho',
                details: { productId, requestedQuantity: totalQty },
              });
              ...
              movementInputs.push({
                ...
                type:
                  item.lineType === InvoiceLineType.GIFT
                    ? directGiftLines.length
                      ? InventoryMovementType.INVOICE_GIFT_FROM_WAREHOUSE
                      : InventoryMovementType.PROMOTION_GIFT_FROM_WAREHOUSE
                    : InventoryMovementType.WAREHOUSE_SALE,
          ...
          const truck = await this.truckModel
            .findOne({ _id: dto.truckId, isDeleted: false })
            .session(session);
        ...
        const paymentDueDate = dto.paymentDueDate
          ...
          : dto.paymentTermDays
            ? new Date(date.getTime() + dto.paymentTermDays * 86400000)
            : undefined;
        const invoice: any = (
            ...
            [
              {
                ...
                giftCode,
                date,
                customer: customer?.name || dto.customer || 'Khách lẻ',
                customerId: customer?._id,
                customerCode: customer?.code,
                customerCodeStatus: customer?.codeStatus,
                ...
                payments: split.invoicePayments,
                paidAmount,
                receivedAmount,
                existingDebtPaidAmount,
                debtAmount,
                initialPaidAmount: paidAmount,
                initialDebtAmount: debtAmount,
                customerDebtBefore: customer ? Number(customer.debt || 0) : 0,
                customerDebtAfter,
                paymentStatus,
                paymentDueDate,
                paymentTermDays: dto.paymentTermDays,
                debtLimitOverridden: Boolean(dto.allowDebtLimitOverride),
                debtOverrideReason: dto.debtOverrideReason?.trim(),
                promotionId: calculated.promotion?._id,
                ...
                promotionApplications: giftApplication
                  ? [
                      {
                        promotionId: giftApplication.promotion._id,
                        promotionCode: giftApplication.promotion.code,
                        promotionName: giftApplication.promotion.name,
                        applicationCount: giftApplication.applicationCount,
                        matchedConditions: giftApplication.matchedConditions,
                        gifts: giftApplication.gifts,
                      },
                    ]
                  : [],
        ...
        const activation: any =
          giftApplication && customer
            ? await this.activations.createForInvoice(
                {
                  promotion: giftApplication.promotion,
                  invoice,
                  customer,
                  salesperson,
                  date,
        ...
        if (activation)
          await this.model.updateOne(
            {
              _id: invoice._id,
              'promotionApplications.promotionId':
                giftApplication.promotion._id,
            },
            {
              $set: {
                'promotionApplications.$.activationId': String(activation._id),
                'promotionApplications.$.activationCode': activation.code,
              },
            },
            { session },
          );
        ...
        if (giftApplication)
          await this.promotionModel.updateOne(
            { _id: giftApplication.promotion._id },
            { $inc: { used: 1 } },
            { session },
          );
        ...
        if (customer && (debtAmount > 0 || existingDebtPaidAmount > 0)) {
          ...
          if (!dto.allowDebtLimitOverride && customer.debtLimit > 0)
            debtFilter.$expr = {
              $lte: [{ $add: ['$debt', netDebtChange] }, '$debtLimit'],
            };
          const debtUpdated = await this.customerModel.findOneAndUpdate(
            debtFilter,
            { $inc: { debt: netDebtChange } },
            { new: true, session },
          );
          if (!debtUpdated)
            throw new ConflictException({
              code: 'CUSTOMER_DEBT_CHANGED',
              message: 'Công nợ khách hàng vừa thay đổi, vui lòng thử lại',
            });
          if (debtAmount > 0) {
              ...
              [
                {
                  customerId: customer._id,
                  customerCode: customer.code,
                  type: DebtLedgerType.INVOICE_DEBT,
                  direction: DebtLedgerDirection.INCREASE,
                  amount: debtAmount,
                  previousDebt: customer.debt || 0,
                  increaseAmount: debtAmount,
                  decreaseAmount: 0,
                  balanceAfter: Number(customer.debt || 0) + debtAmount,
          ...
          if (existingDebtPaidAmount > 0) {
            const collector: any = await this.userModel
              .findOne({
                _id: actor.id,
                isDeleted: false,
            ...
            if (!collector)
              throw new ForbiddenException(
                'Tài khoản người thu không còn hoạt động',
              );
            const receiptCounter: any =
              await this.debtPaymentCounterModel.findOneAndUpdate(
                { key: `DEBT_PAYMENT_${day.slice(2)}` },
                { $inc: { sequence: 1 } },
                { upsert: true, new: true, session },
              );
            debtPaymentCode = `PTCN-${day.slice(2)}-${String(receiptCounter.sequence).padStart(6, '0')}`;
            const receiptId = new Types.ObjectId();
            let remaining = existingDebtPaidAmount;
            const allocations: any[] = [];
            const oldInvoices: any[] = await this.model
              .find({
                customerId: customer._id,
                debtAmount: { $gt: 0 },
                isDeleted: false,
            ...
            for (const oldInvoice of oldInvoices) {
              ...
              const debtBefore = Number(oldInvoice.debtAmount || 0);
              const allocated = Math.min(remaining, debtBefore);
              const debtAfter = debtBefore - allocated;
              const paidAfter = Number(oldInvoice.paidAmount || 0) + allocated;
              ...
              if (result.modifiedCount !== 1)
                throw new ConflictException(
                  'Công nợ hóa đơn cũ vừa thay đổi, vui lòng thử lại',
                );
              allocations.push({
                invoiceId: oldInvoice._id,
                invoiceCode: oldInvoice.code,
                amount: allocated,
                debtBefore,
                debtAfter,
              });
            ...
            await this.debtPaymentModel.create(
              [
                {
                  _id: receiptId,
                  code: debtPaymentCode,
                  date,
                  customerId: customer._id,
                  customerCode: customer.code,
                  customerName: customer.name,
                  customerPhone: customer.phone || '',
                  amount: existingDebtPaidAmount,
                  payments: split.debtPayments,
                  allocations,
                  unallocatedAmount: remaining,
                  customerDebtBefore: customer.debt || 0,
                  customerDebtAfter: Math.max(
                    0,
                    Number(customer.debt || 0) - existingDebtPaidAmount,
                  ),
                  note: `Thu nợ cũ cùng hóa đơn ${code}`,
            ...
            await this.model.updateOne(
              { _id: invoice._id },
              { $set: { debtPaymentId: String(receiptId), debtPaymentCode } },
              { session },
            );
            await this.debtLedgerModel.create(
              [
                {
                  ...
                  amount: existingDebtPaidAmount,
                  previousDebt: Number(customer.debt || 0) + debtAmount,
                  increaseAmount: 0,
                  decreaseAmount: existingDebtPaidAmount,
                  balanceAfter: customerDebtAfter,
                  previousDebtLimit: customer.debtLimit || 0,
                  debtLimitAfter: customer.debtLimit || 0,
                  occurredAt: date,
                  effectiveAt: date,
          ...
          data: {
            ...
            customer: customer
              ? {
                  ...
                  codeStatus:
                    customer.codeStatus || CustomerCodeStatus.UNASSIGNED,
                  name: customer.name,
                }
              : null,
            items: inventoryLines,
      ...
      await this.notifications
        .create({
          ...
          title: 'Hóa đơn mới',
          message: `Đã tạo hóa đơn ${response.data.code}`,
          entityType: 'INVOICE',
          entityId: response.data.id,
          entityCode: response.data.code,
          staffRecipientId: salespersonId,
        })
        .catch(() => undefined);
      return response;
    } finally {
  ...
  async reverse(
    ...
    if (actor.role !== RoleEnum.ADMIN || !actor.id)
      throw new ForbiddenException('Chỉ quản trị viên được hoàn hóa đơn');
    const session = await this.connection.startSession();
    let response: any;
    try {
      await session.withTransaction(async () => {
        ...
        if (!invoice)
          throw new ConflictException({
            code: 'INVOICE_ALREADY_REVERSED_OR_NOT_FOUND',
            message: 'Hóa đơn không tồn tại hoặc đã được hoàn',
          });
        const linkedReceiptId = invoice.debtPaymentId
          ? String(invoice.debtPaymentId)
          : undefined;
        const allocatedReceipt = await this.debtPaymentModel
          .findOne({
            isDeleted: false,
        ...
        if (allocatedReceipt)
          throw new ConflictException({
            code: 'INVOICE_HAS_DEBT_PAYMENT_ALLOCATIONS',
            message: 'Hãy hủy phiếu thu đã phân bổ cho hóa đơn trước khi hoàn',
          });
        ...
        for (const item of invoice.items || []) {
          if (invoice.sourceType === 'warehouse') {
            ...
            if (!before)
              throw new ConflictException('Không thể hoàn hàng về kho');
            reversalMovements.push({
              productId: item.productId,
              type: InventoryMovementType.INVOICE_REVERSAL_TO_WAREHOUSE,
              quantityChange: item.qty,
              quantityBefore: before.stock,
              quantityAfter: before.stock + item.qty,
            ...
            if (!truck)
              throw new ConflictException(
                'Xe nguồn của hóa đơn không còn tồn tại',
              );
            const entry = (truck.inventory || []).find(
              (value) => String(value.productId) === String(item.productId),
            );
            const quantityBefore = Number(entry?.qty || 0);
            if (entry)
              await this.truckModel.updateOne(
                { _id: truck._id, 'inventory.productId': item.productId },
            ...
            reversalMovements.push({
              productId: item.productId,
              type: InventoryMovementType.INVOICE_REVERSAL_TO_TRUCK,
              quantityChange: item.qty,
              quantityBefore,
              quantityAfter: quantityBefore + item.qty,
          ...
          linkedReceipt = await this.debtPaymentModel.findOneAndUpdate(
            ...
            },
            {
              $set: {
                ...
                cancelReason: `Hoàn hóa đơn ${invoice.code}: ${reason}`,
              },
            },
            { new: false, session },
          );
          if (linkedReceipt) {
            for (const allocation of linkedReceipt.allocations || []) {
              const old: any = await this.model
                .findOne({ _id: allocation.invoiceId, isDeleted: false })
                .session(session);
              if (!old)
                throw new ConflictException(
                  'Không thể khôi phục công nợ hóa đơn từ phiếu thu liên kết',
                );
              const debtAmount =
                Number(old.debtAmount || 0) + allocation.amount;
              const paidAmount = Math.max(
                0,
                Number(old.paidAmount || 0) - allocation.amount,
              );
              await this.model.updateOne(
                { _id: old._id },
                {
                  ...
                  $pull: { debtPayments: { receiptId: linkedReceiptId } },
                },
                { session },
              );
            }
        ...
        if (invoice.customerId && netDebtChange !== 0) {
          const customer: any = await this.customerModel.findOneAndUpdate(
            {
              ...
              debt: {
                $gte: Math.max(
                  0,
                  remainingInvoiceDebt - debtIncreaseFromReceipt,
                ),
              },
            },
            { $inc: { debt: netDebtChange } },
          ...
          if (!customer)
            throw new ConflictException(
              'Không thể cập nhật công nợ khi hoàn hóa đơn',
            );
          await this.debtLedgerModel.create(
            [
              {
                ...
                type: DebtLedgerType.ADJUSTMENT,
                direction:
                  netDebtChange > 0
                    ? DebtLedgerDirection.INCREASE
                    : DebtLedgerDirection.DECREASE,
                amount: Math.abs(netDebtChange),
                previousDebt: customer.debt - netDebtChange,
                increaseAmount: Math.max(0, netDebtChange),
                decreaseAmount: Math.max(0, -netDebtChange),
        ...
        await this.activationModel.updateMany(
          ...
          {
            $set: {
              status: PromotionActivationStatus.REVOKED,
              statusReason: reason,
        ...
        await this.model.updateOne(
          { _id: invoice._id },
          { $set: { reversalCode }, $unset: { giftCode: 1 } },
          { session },
        );
        await this.movements.recordMany(
      ...
      await this.notifications
        .create({
          type: NotificationType.INVOICE_REVERSED,
          title: 'Hoàn hóa đơn',
          message: `Đã hoàn hóa đơn ${response.data.code}`,
          entityType: 'INVOICE',
          entityId: response.data.id,
          entityCode: response.data.code,
        })
  ...
  private async resolveInvoiceReadScope(actor: Actor) {
    if (!actor.id || !Types.ObjectId.isValid(actor.id))
      throw new ForbiddenException('Không xác định được tài khoản hiện tại');
    const current: any = await this.userModel
      .findOne({
        _id: actor.id,
        isDeleted: false,
        status: { $ne: UserStatus.INACTIVE },
      })
      .select('_id role canViewAllInvoices')
      .lean();
    ...
    if (query.search?.trim()) {
      ...
      const [customers, activations] = await Promise.all([
        ...
        this.activationModel
          .find({ isDeleted: false, code: regex })
          .select('invoiceId')
          .limit(500)
          .lean(),
      ]);
      filter.$or = [
        ...
        { _id: { $in: activations.map((item: any) => item.invoiceId) } },
        { 'promotionApplications.activationCode': regex },
      ];
    }
    return filter;
  }
  ...
  async findAll(query: InvoiceQueryDto = {}, actor: Actor = {}): Promise<any> {
    ...
    const activations: any[] = invoiceIds.length
      ? await this.activationModel
          ...
          .select('invoiceId code status activatedAt')
          .lean()
      : [];
    const byInvoice = new Map<string, any[]>();
    for (const activation of activations) {
      const key = String(activation.invoiceId);
  ...
  async timeline(query: InvoiceQueryDto = {}, actor: Actor = {}): Promise<any> {
    const { invoiceFilter, receiptFilter } = await this.timelineFilters(
      query,
      actor,
    );
    const page = Math.max(1, Number(query.page) || 1);
    const limit = Math.min(100, Math.max(1, Number(query.limit) || 20));
    const documents = await this.timelineDocuments(
      invoiceFilter,
      receiptFilter,
    );
  ...
  private async timelineFilters(
    ...
    if (salespersonId) {
      ...
      receiptFilter.collectorId = salespersonId;
    }
  ...
  private async timelineDocuments(
    invoiceFilter: any,
    receiptFilter: any,
  ): Promise<any[]> {
    ...
    const documents = [
      ...
      return (
        dateDifference || String(right._id).localeCompare(String(left._id))
      );
    });
    return documents;
  }
  ...
  async export(
    ...
    const invoicesSheet = workbook.addWorksheet('Danh sách hóa đơn', {
      views: [{ state: 'frozen', ySplit: 1 }],
    });
    invoicesSheet.columns = [
      { header: 'STT', key: 'number', width: 7 },
      { header: 'Loại chứng từ', key: 'documentType', width: 22 },
      { header: 'Mã chứng từ', key: 'code', width: 22 },
      { header: 'Ngày giờ', key: 'date', width: 20 },
      { header: 'Mã khách hàng', key: 'customerCode', width: 17 },
      ...
      { header: 'Hàng hóa', key: 'products', width: 48 },
      { header: 'Tổng tiền hàng', key: 'grandTotal', width: 18 },
      { header: 'Số tiền thanh toán (3)', key: 'receivedAmount', width: 22 },
      { header: 'Nợ cũ (2)', key: 'customerDebtBefore', width: 16 },
      { header: 'Công nợ còn lại', key: 'customerDebtAfter', width: 20 },
    ...
    documents.forEach((doc: any, index) => {
      const receipt = doc.documentType === 'DEBT_PAYMENT';
      const customer: any =
        doc.customerId && typeof doc.customerId === 'object'
          ? doc.customerId
          : {};
      const items: any[] = receipt
        ? [
            {
              ...
              qty: 1,
              price: 0,
              lineTotal: Number(doc.amount || 0),
              lineType: InvoiceLineType.SALE,
            },
          ]
        : doc.items || [];
      const reversed =
        doc.status === InvoiceStatus.REVERSED || Boolean(doc.reversedAt);
      const receivedAmount = receipt
        ? Number(doc.amount || 0)
        : Number(doc.receivedAmount ?? doc.paidAmount ?? 0);
      invoicesSheet.addRow({
        number: index + 1,
        documentType: receipt ? 'Thanh toán công nợ' : 'Hóa đơn bán hàng',
        code: doc.code || '',
        date: new Date(doc.date || doc.createdAt),
        customerCode: doc.customerCode || customer.code || '',
        customerName:
          doc.customerName || customer.name || doc.customer || 'Khách lẻ',
        customerPhone: doc.customerPhone || customer.phone || '',
        employee:
          doc.salespersonName ||
          doc.collectorName ||
          doc.salespersonId?.fullName ||
          '',
        products: items
          .map(
            (item) =>
              `${item.productName || 'Sản phẩm'} x ${Number(item.qty || 0)}${item.lineType === InvoiceLineType.GIFT ? ' (Quà tặng)' : ''}`,
          )
          .join('; '),
        grandTotal: receipt
          ? 0
          : Number(doc.grandTotal ?? doc.totalAmount ?? 0),
        receivedAmount,
        customerDebtBefore: Number(doc.customerDebtBefore || 0),
        customerDebtAfter: Number(
          doc.customerDebtAfter ?? (receipt ? 0 : doc.debtAmount) ?? 0,
        ),
        status: reversed
          ? 'Đã hoàn'
          : receipt
            ? doc.status === DebtPaymentStatus.CANCELLED
              ? 'Phiếu thu đã hủy'
              : 'Đã thu công nợ'
            : doc.paymentStatus === InvoicePaymentStatus.PAID
              ? 'Đã thanh toán'
              : doc.paymentStatus === InvoicePaymentStatus.PARTIAL
                ? 'Thanh toán một phần'
                : 'Chưa thanh toán',
        note: doc.note || '',
      });
      for (const item of items) {
        ...
        productRows.push({
          ...
          catalogPrice: Number(
            item.catalogPrice ?? item.originalPrice ?? item.price ?? 0,
          ),
          price: Number(item.price || 0),
          lineTotal: Number(item.lineTotal || 0),
          invoiceTotal: receipt
            ? 0
            : Number(doc.grandTotal ?? doc.totalAmount ?? 0),
          receivedAmount,
          customerDebtAfter: Number(
            doc.customerDebtAfter ?? doc.debtAmount ?? 0,
          ),
        });
      }
    });
    const productsSheet = workbook.addWorksheet('Sản phẩm đã bán', {
      views: [{ state: 'frozen', ySplit: 1 }],
    });
    productsSheet.columns = [
      ...
      { header: 'Giá niêm yết', key: 'catalogPrice', width: 18 },
    ...
    for (const sheet of [invoicesSheet, productsSheet]) {
    ...
    invoicesSheet.getColumn('date').numFmt = 'dd/mm/yyyy hh:mm';
    productsSheet.getColumn('date').numFmt = 'dd/mm/yyyy hh:mm';
    for (const key of [
      'grandTotal',
      'receivedAmount',
      'customerDebtBefore',
      ...
      invoicesSheet.getColumn(key).numFmt = '#,##0';
    for (const key of [
      'quantity',
      'catalogPrice',
      'price',
      ...
      productsSheet.getColumn(key).numFmt = '#,##0';
    return Buffer.from(await workbook.xlsx.writeBuffer());
  }

  async summary(query: InvoiceQueryDto, actor: Actor = {}): Promise<any> {
    const filter = await this.invoiceFilter(
      { ...query, search: undefined, page: undefined, limit: undefined },
      actor,
    );
    const [values, customers, activationCount] = await Promise.all([
      ...
      this.model.distinct('customerId', {
        ...filter,
        customerId: { $ne: null },
      }),
      this.activationModel.countDocuments({
        ...
        ...(query.from || query.to
          ? {
              activatedAt: {
                ...(query.from
                  ? { $gte: vietnamDateBoundary(query.from, false) }
                  : {}),
                ...(query.to
                  ? { $lte: vietnamDateBoundary(query.to, true) }
                  : {}),
              },
            }
          : {}),
    ...
    return {
      data: {
        ...
        promotionActivationCount: activationCount,
      },
    };
  }
