...
import { ImportStatus } from '../imports/schemas/imports.schema';
...
export class ReportsService {
  ...
  constructor(
    ...
    private customersModel: ReturnModelType<typeof Customers>,
  ) {}
  ...
  async inventory(query: DashboardPeriodQueryDto): Promise<any> {
    ...
    const [products, imports, movements] = await Promise.all([
      ...
      this.imports
        .find({
          isDeleted: false,
          status: { $in: [ImportStatus.RECEIVED, 'completed'] },
          date: {
    ...
    return {
      ...
      summary: {
        ...
        totalAmount: data
          .filter(
            (row) =>
              row.status === ImportStatus.RECEIVED ||
  ...
  async customers(
    ...
    if (query.search?.trim()) {
      const escaped = query.search
        .trim()
        .replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
      searchFilter.$or = ['code', 'name', 'phone', 'phones'].map((field) => ({
    ...
    const [customers, invoices, receipts]: any[][] = await Promise.all([
      ...
      this.invoices
        ...
        .select('customerId grandTotal initialDebtAmount debtAmount payments')
        .lean(),
      this.debtPayments
        .find({
          isDeleted: false,
    ...
    for (const invoice of invoices) {
      const m = get(String(invoice.customerId));
      m.invoiceCount += 1;
      m.purchaseAmount += Number(invoice.grandTotal || 0);
      m.debtAddedAmount +=
        invoice.initialDebtAmount !== undefined
          ? Number(invoice.initialDebtAmount || 0)
          : Number(invoice.debtAmount || 0);
      m.cashPaidAmount += (invoice.payments || [])
        .filter((p) => p.method === PaymentMethod.CASH)
        .reduce((sum, p) => sum + Number(p.amount || 0), 0);
    }
    for (const receipt of receipts) {
      const m = get(String(receipt.customerId));
      m.cashPaidAmount += (receipt.payments || [])
        .filter((p) => p.method === PaymentMethod.CASH)
        .reduce((sum, p) => sum + Number(p.amount || 0), 0);
    }
    const allRows = customers.map((customer: any) => {
      const m = metrics.get(String(customer._id)) || {
        invoiceCount: 0,
        purchaseAmount: 0,
        debtAddedAmount: 0,
        cashPaidAmount: 0,
      };
      const hasPurchased = m.invoiceCount > 0;
      return {
    ...
    const filtered =
      status === 'PURCHASED'
        ...
        : status === 'NOT_PURCHASED'
          ? allRows.filter((row) => !row.hasPurchased)
          : allRows;
    const page = Math.max(1, Number(query.page) || 1),
      limit = Math.min(100, Math.max(1, Number(query.limit) || 20));
    const visible = exportAll
      ? filtered
      : filtered.slice((page - 1) * limit, page * limit);
    const data = visible.map((row, index) => ({
      ...row,
      rowNumber: exportAll ? index + 1 : (page - 1) * limit + index + 1,
    }));
    return {
      data: {
        period,
        summary: {
          ...
          customersWithInvoices: purchased.length,
          customersWithoutInvoices: allRows.length - purchased.length,
          invoiceCount: allRows.reduce((s, x) => s + x.invoiceCount, 0),
          purchaseAmount: allRows.reduce((s, x) => s + x.purchaseAmount, 0),
          debtAddedAmount: allRows.reduce((s, x) => s + x.debtAddedAmount, 0),
          cashPaidAmount: allRows.reduce((s, x) => s + x.cashPaidAmount, 0),
        },
        data,
        meta: {
          page: exportAll ? 1 : page,
          limit: exportAll ? filtered.length : limit,
  ...
  private async exportCustomers(
    ...
    for (const row of result.data.data || [])
      detail.addRow({
        ...row,
        purchaseStatusLabel: row.hasPurchased
          ? 'Có mua hàng'
          : 'Không mua hàng',
      });
    ...
    filters.columns = [
      { header: 'Bộ lọc', key: 'key', width: 30 },
      { header: 'Giá trị', key: 'value', width: 45 },
    ];
    filters.addRows([
      {
        key: 'Kỳ báo cáo',
        value: query.period || result.data.period?.type || 'MONTH',
      },
      {
        key: 'Từ ngày',
        value: result.data.period?.from
          ? new Date(result.data.period.from).toLocaleString('vi-VN', {
              timeZone: 'Asia/Ho_Chi_Minh',
            })
          : '',
      },
      {
        key: 'Đến ngày',
        value: result.data.period?.to
          ? new Date(result.data.period.to).toLocaleString('vi-VN', {
              timeZone: 'Asia/Ho_Chi_Minh',
            })
          : '',
      },
