...
export enum LeadInteractionResult { VISITED = 'VISITED', SOLD = 'SOLD', NO_SALE = 'NO_SALE' }
...
export class LeadInteraction {
  ...
  @prop({ required: true, enum: LeadInteractionResult }) result: LeadInteractionResult;
...
export class Leads extends BaseModel {
  ...
  @prop({ type: () => LeadLocation, _id: false, required: true }) location: LeadLocation;
  ...
  @prop({ type: () => [LeadInteraction], default: [] }) interactions: LeadInteraction[];
}
